...
public class Script {
    ...
    public boolean run() {
        try {
            ...
            final File file = new File(Rug.scriptManager.tempDir);
            ...
            final StandardJavaFileManager standardFileManager = Rug.scriptManager.compiler.getStandardFileManager(bp, null, null);
            final ArrayList<String> list = new ArrayList<>();
            list.add("-d");
            list.add(Rug.scriptManager.tempDir);
            list.add("-XDuseUnsharedTable");
            if (!(boolean) Launch.blackboard.get("fml.deobfuscatedEnvironment")) {
                list.add("-classpath");
                String s = Rug.scriptManager.b;
                try {
            ...
            boolean success = Rug.scriptManager.compiler.getTask(null, standardFileManager, bp, list, null, Collections.singletonList(new ClassObject(this.scriptName, this.codeStr, this.extraLines))).call();
            if (!success) {
                this.error = true;
                return false;
            }
    ...
    public void delete() {
        this.d = null;
        this.b = null;
        final File file = new File(Rug.scriptManager.tempDir + File.separator + this.scriptName + ".class");
        if (file.exists()) {
            file.delete();
        }
    }
    ...
    public void createScript(final String s) {
        ...
        final Iterator<String> iterator = Rug.scriptManager.imports.iterator();
